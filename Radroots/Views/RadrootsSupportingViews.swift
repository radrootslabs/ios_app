        ...
        List {
            ...
            Section("My posts") {
                ...
                ForEach(store.snapshot?.cards ?? []) { card in
                    NavigationLink {
                        RadrootsTodayDetailView(card: card, canRevise: true, revise: revise)
                    } label: {
                        RadrootsTodayCardView(card: card)
                    }
                }
            ...
            Section("Nostr relays") {
                ...
                ForEach(snapshot.relay?.relays ?? [], id: \.url) { relay in
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        Text("\(relay.access.replacingOccurrences(of: "_", with: " ")) · read \(relay.readState) · write \(relay.writeState)")
                            .font(.caption2)
                            .foregroundStyle(.secondary)
                    }
                }
